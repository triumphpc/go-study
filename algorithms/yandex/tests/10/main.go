...
func task(in io.Reader, out io.Writer) {
	...
	for num := 0; num < size; num++ {
		...
		currentLen++ // Каждый раз увеличиваем расстояние

		if houseNum == 0 { // Если нашли нулевой
			currentLen = 0 // Сбрасываем длину
			if num > 1 {
				houseCrd[num-1] = 1 // Влево возвращаем и обновляем шаги
			}

			if leftIdx == 0 { // Выбираем левый индекс для подсчета шага назад
				currentLeft = 0
			} else {
				currentLeft = leftIdx + ((num - leftIdx) / 2) // Определяем левую грницу до нуля и берем половину
			}

			for j := num - 2; j >= currentLeft; j-- {
				houseCrd[j] = houseCrd[j+1] + 1 // А тут проставляем на оди больше от правой части отрезка
			}
